...
import {
  singleWorkout,
  singleHistoryWorkout,
  allWorkouts,
  allHistoryWorkouts,
} from '../../../Types/Interfaces/InterfecesList'
//enums
...
export default class MainPanel extends React.Component<IDetailProps, IDetailState> {
  ...
  handleConfirmWorkout = (data: singleWorkout, undo: boolean = false) => {
    ...
    if (whichWorkout !== null) {
      axios
        .put<singleWorkout>(
          `https://sportplan-addc3.firebaseio.com/Users/${this.context.userDatabaseId}/workoutType/${whichWorkout}.json${this.context.userAuthQuery}`,
          data
        )
        .then((response) => {
  ...
  handleLvlUpButton = (workout: singleWorkout) => {
    const data = WorkoutMethods.createSingleHistoryWorkoutObject(
      workout.level,
      workout.numberOfSeries,
      workout.quantityInSeries
    )

    const resetData: singleWorkout = WorkoutMethods.createSingleWorkoutObject(
      workout.id,
      workout.level + 1,
      workout.name,
      [0],
    ...
    axios
      .get(
        `https://sportplan-addc3.firebaseio.com/Users/${this.context.userDatabaseId}/workoutTypeHistory/${whichWorkout}.json${this.context.userAuthQuery}`
      )
      .then((response) => {
        let exist: boolean | null = null
        let backupLvl: singleHistoryWorkout | null = WorkoutMethods.createSingleHistoryWorkoutObject(
          1,
          [0],
          [0]
        )
        ...
        if (exist === true && backupLvl !== null) {
          const oldData = WorkoutMethods.createSingleWorkoutObject(
            workout.id,
            backupLvl.level,
            workout.name,
            backupLvl.numberOfSeries,
            backupLvl.quantityInSeries
          )

          axios
            .put<singleHistoryWorkout>(
              `https://sportplan-addc3.firebaseio.com/Users/${this.context.userDatabaseId}/workoutTypeHistory/${whichWorkout}/${whichLvl}.json${this.context.userAuthQuery}`,
              data
            )
            .then((response) => {
              axios
  ...
  handleLvlDownButton = (workout: singleWorkout) => {
    ...
    const dataToWrite: singleHistoryWorkout = WorkoutMethods.createSingleHistoryWorkoutObject(
      workout.level,
      workout.numberOfSeries,
      workout.quantityInSeries
    )
    ...
    axios
      ...
      .then((response) => {
        axios
          .get<allHistoryWorkouts>(
            `https://sportplan-addc3.firebaseio.com/Users/${this.context.userDatabaseId}/workoutTypeHistory.json${this.context.userAuthQuery}`
          )
          .then((response) => {
                  ...
                  if (level[0] === `lvl${workout.level - 1}`) {
                    ...
                    axios
                      ...
                      .then((response) => {
                        this.handleMessage(
                          workout.id,
                          'Przywrócono poprawnie niższy poziom',
                          MessageType.GOOD,
                          true
                        )
                      })
  ...
  render() {
    ...
    return (
      <>
        <button
          className="workout__goBackButton"
          onClick={() => {
