...
export const ChoosenWorkoutInfo: React.FC<IDetailProps> = (props) => {
  ...
  for (let i = 1; i <= 10; i++) {
    //if (singleWorkout.level >= i) reachLvl = true
    //else reachLvl = false
    ...
    <Wrapper>
      ...
      <MainContainer>
        ...
        <ContentWrapper areaId={1}>
          ...
          wysokości klatki piersiowej. To jest pozycja wyjściowa (fot. 1). Zginaj ramiona i łokcie,
          aż delikatnie dotkniesz czołem do ściany. To jest pozycja końcowa (fot. 2). Odepchnij się
          do tyłu do pozycji wyjściowej i powtórz.
        </ContentWrapper>
        ...
        <ContentWrapper areaId={2}>
          ...
          władanie nad swoim ciałem, powinien być w stanie wykonać ją bez problemu. Jest to również
          pierwszy element w sekwencji terapeutycznej pompek. Ćwiczenie to jest bardzo korzystne dla
          osoby odzyskującej sprawność po kontuzji lub operacji, kiedy dąży do wzmocnienia zdrowia i
          stopniowego odbudowania siły. Łokcie, nadgarstki i barki (zwłaszcza delikatne mięśnie
          pierścienia rotatorów) są szczególnie narażone na ostre lub przewlekłe urazy. Ćwiczenie to
          łagodnie aktywizuje te obszary, pobudzając je oraz zwiększając ukrwienie i napięcie
          toniczne. Nowicjusze w sztuce kalisteniki powinni wchodzić w każdy program treningowy
          bardzo płynnie, tak by wyrobić odpowiednią technikę i umiejętności. Dlatego nie wolno
          pomijać tego ćwiczenia.
        </ContentWrapper>
        <TitleWrapper areaId={3}>
          <Goal />
        </TitleWrapper>
        <ContentWrapper areaId={3}>
          Próg początkowy: 1 seria 10 powtórzeń Próg średniozaawansowany: 2 serie po 25 powtórzeń
          Próg przejścia: 3 serie po 50 powtórzeń
        </ContentWrapper>
