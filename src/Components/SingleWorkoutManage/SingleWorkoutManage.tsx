...
const SingleWorkoutManage: React.FC<IDetailProps> = (props) => {
  const lastNumberOfSeries: number =
    props.workout.numberOfSeries[props.workout.numberOfSeries.length - 1]
  const lastquantityInSeries: number =
    props.workout.quantityInSeries[props.workout.quantityInSeries.length - 1]
  ...
  const handleButton = () => {
    ...
    let data: singleWorkout = WorkoutMethods.createSingleWorkoutObject(
      props.workout.id,
      props.workout.level,
      props.workout.name,
      newNumberOfSeries,
  ...
  let checkStatus = (status: UpOrDown) => {
    ...
    Object.entries(lvlUpData).forEach((item) => {
      if (item[1].id === id) {
        if (
          (lastquantityInSeries >= item[1].lvlUpQuantity[level - 1] &&
            lastNumberOfSeries >= item[1].lvlUpSeries[level - 1]) ||
          lastquantityInSeries * lastNumberOfSeries >=
            item[1].lvlUpQuantity[level - 1] * item[1].lvlUpSeries[level - 1]
        ) {
          flag = false
        }
      }
  ...
  if (props.message !== null) {
    if (props.message.id === props.workout.id)
      message = <MessageBox type={props.message.type}>{props.message.text}</MessageBox>
  }
    ...
    disableWindow = (
      <Backdrop>
        <button
          ...
          className="singleWorkout__current__button singleWorkout__current__button--undo"
        >
          Przywróć
        </button>
      </Backdrop>
      ...
      <div className="singleWorkout">
        ...
        <div className="singleWorkout__last">
          <div className="singleWorkout__last__text">
            ...
            <h6>{levelText}</h6>
            <h6>
              Ilosc: {lastNumberOfSeries} x {lastquantityInSeries}
              {displaySeconds}
            </h6>
          </div>
          <WorkoutSvg
            className="singleWorkout__last__image"
            id={props.workout.id as WhichWorkout}
          />
        </div>
        ...
        <div className="singleWorkout__current">
          ...
          <div className="singleWorkout__current__level">
            <button
              className={buttonDownValue}
              disabled={checkStatus(UpOrDown.DOWN)}
              onClick={() => props.handleLvlDownButton(props.workout)}
            >
            ...
            <button
              className={buttonUpValue}
              disabled={checkStatus(UpOrDown.UP)}
              onClick={() => props.handleLvlUpButton(props.workout)}
            >
          ...
          <div className="singleWorkout__current__numberOfSeries">
            <button
              className="singleWorkout__current__button"
              disabled={windowStatus}
              onClick={() => {
                if (currentNumber > 0) changeCurrentNumber(currentNumber - 1)
              }}
            >
            ...
            <button
              className="singleWorkout__current__button"
              disabled={windowStatus}
              onClick={() => changeCurrentNumber(currentNumber + 1)}
            >
          ...
          <div className="singleWorkout__current__numberOfSeries">
            <button
              ...
              disabled={windowStatus}
              onClick={() => {
                if (currentQuantity > 0) changeQuantity(currentQuantity - 1)
              }}
            >
              <FontAwesomeIcon className="singleWorkout__current__button__icon" icon={faMinus} />
            </button>
            <h6>
              Ilość {repeatingText}: {currentQuantity}{" "}
            </h6>
            <button
              className="singleWorkout__current__button"
              disabled={windowStatus}
              onClick={() => changeQuantity(currentQuantity + 1)}
            >
              <FontAwesomeIcon className="singleWorkout__current__button__icon" icon={faPlus} />
            </button>
          </div>
